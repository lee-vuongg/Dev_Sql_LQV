...
And [MaSV] Not In (select [MaSV] from [dbo].[Ketqua]
						where [MaMH] in
						(select [MaMH] from [dbo].[MonHoc] where [TenMH] = N'Cơ Sở Dữ Liệu'))
--Dien gia
--Sinh vien cau khoa anh van
SELECT MaSV, HoSV, TenSV, Phai, NgaySinh, NoiSinh, MaKH, HocBong, DiemTrungBinh
from [dbo].[SinhVien]
where [MaKH] in (select [MaKH] from [dbo].[Khoa] where [TenKH] =N'Anh văn')
--SV đã thi môn Cơ sở dữ liệu
select MaSV from [dbo].[Ketqua] where [MaMH] in (select [MaMH] from [dbo].[MonHoc] where [TenMH] = N'Cơ Sở Dữ Liệu')
--SV Chua thi môn Cơ sở dữ liệu
...
--Bai 3: cho biết môn nào có điểm thi cao nhất, gồm các thông tin : tên môn , tên sinh viên, số tiết ,điểm
...
order by Diem Desc 
--Các sinh viên có học bổng cao nhất theo từng khoa, gồm Mã sinh viên, Tên khoa,  Học bổng
select Top 10 with Ties [TenKH],[MaSV],[HocBong]  from [dbo].[SinhVien] SV 
inner join [dbo].[Khoa] K on SV.MaKH = K.MaKH
order by [HocBong] Desc 
 --Cho biết những sinh viên có học bổng lớn hơn tổng học bổng của những sinh  viên thuộc khoa Triết 
 --tìm sv khoa triết
 select SUM([HocBong]) from [dbo].[SinhVien]
 where [MaKH] in ( select [MaKH] from [dbo].[Khoa] where [TenKH] = N'Kế Toán')
 Select * from [dbo].[SinhVien] where
  [HocBong]>( select SUM([HocBong])from [dbo].[SinhVien] where [MaKH] in ( select [MaKH] from [dbo].[Khoa] where [TenKH] = N'Kế Toán')) 
--Liệt kê những sinh viên sinh sau sinh viên có tuổi nhỏ nhất trong khoa Anh văn
select MaSV, HoSV, TenSV, NgaySinh, NoiSinh from [dbo].[SinhVien]
 where [MaKH] in (select [MaKH] from [dbo].[Khoa] where [TenKH] =N'Anh văn')
 --sinh viên có tuổi nhỏ nhất trong khoa Anh văn
 select  NgaySinh from [dbo].[SinhVien]
 where [MaKH] in (select [MaKH] from [dbo].[Khoa] where [TenKH] =N'Anh văn')
 order by NgaySinh Desc
 --1-2
 select MaSV, HoSV, TenSV, NgaySinh from [dbo].[SinhVien] where 
[NgaySinh] >'2000/01/01'(select MIN(NgaySinh) from [dbo].[SinhVien] where [MaKH] in
 (select [MaKH] from [dbo].[Khoa] where [TenKH] = N'Anh văn'))
--Danh sách sinh viên có điểm cao nhất ứng với mỗi môn, gồm thông tin: Mã sinh  viên, Họ tên sinh viên, Tên môn, Điểm 
select Top 1 with Ties  SV.MaSV, concat( HoSV,'', TenSV),Diem,MH.TenMH from [dbo].[SinhVien] SV
inner join [dbo].[MonHoc] MH on MH.TenMH =MH.TenMH
inner join [dbo].[Ketqua] KQ on SV.MaSV = KQ.MaSV 
inner join [dbo].[Khoa] K on SV.MaKH = K.MaKH  
order by [Diem] desc
--.Các sinh viên có học bổng cao nhất theo từng khoa, gồm Mã sinh viên, Tên khoa,  Học bổng 
Select  [MaSV],[HocBong],[TenKH] from [dbo].[SinhVien] SV 
inner join [dbo].[Khoa] K on SV.MaKH=K.MaKH
order by [HocBong] Desc
